...
namespace sfv2 {
    ...
    static const uint16_t crc16_table_[] =
    {
    ...
    static uint16_t crc16(const char* data, int len)
    {
        ...
        while (len--) {
            tmp = crc ^ *data++;
            crc >>= 8;
            crc ^= crc16_table_[tmp];
        }

        // Note that the bytes of the check value are swapped in this scheme
        return ((crc & 0xFF) << 8) | (crc >> 8);
    }

    void ModBus::preset_single_register(uint8_t slave_address,
                                        ...
                                        QByteArray& buffer)
    {
        ...
        const auto crc = crc16(buffer.constData(), 6);

        buffer[6] = crc >> 8;            // CRC
        buffer[7] = crc & 0xFF;
    }

    void ModBus::preset_multiple_registers(uint8_t slave_address,
                                           uint16_t data_address,
                                           uint16_t num_registers,
                                           uint16_t* values,
                                           QByteArray& buffer)
    {
        buffer.resize(9 + num_registers * 2);
        ...
        auto len = 7;
        for (auto i = 0; i < num_registers; i++) {      // Parameter 4: Data bytes
          buffer[len++] = values[i] >> 8;
          buffer[len++] = values[i] & 0xFF;
        }

        const auto crc = crc16(buffer.constData(), len);
